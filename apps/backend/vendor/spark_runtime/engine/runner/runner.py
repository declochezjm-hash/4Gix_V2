...
from readers.gpkg_reader import GpkgReader
...
class Runner:
    ...
    def run_step(self, step: BaseStep, curr_df=None, curr_types={}):
        ...
        elif step.type == 'GpkgReader':
            df, types, output = GpkgReader(**step.options).run()
        elif step.type == 'FilterTransformer':
            df, types, output = FilterTransformer(
