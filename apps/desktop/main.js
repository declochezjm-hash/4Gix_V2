...
async function ensureBackend() {
	...
	const restartInDev = !app.isPackaged && process.env.FOURGIX_KEEP_BACKEND !== "1";
...
function postShutdown() {
	return new Promise((resolve) => {
		const req = http.request(SHUTDOWN_URL, { method: "POST", timeout: 3000 }, (res) => {
			res.resume();
			resolve();
		});
...
ipcMain.handle("dialog:openFile", async (_event, options = {}) => {
	const filters = options.filters || [
		{
			name: "Données SIG",
			extensions: ["shp", "gpkg", "csv", "geojson", "json"],
		},
